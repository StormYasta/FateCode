...
try:
    ...
    if not candidates:
        emit({
            ...
            'stderr': 'Nenhuma função principal foi encontrada. Declare uma função com def nome(...):',
        })
        raise SystemExit(0)
    ...
    emit({
        ...
        'testResults': [],
        'stdout': stdout_buffer.getvalue(),
        'stderr': 'Erro de sintaxe na linha {}: {}'.format(exc.lineno or '?', exc.msg),
    })
except SystemExit:
    pass
except Exception as exc:
    emit({
        ...
        'testResults': [],
        'stdout': stdout_buffer.getvalue(),
        'stderr': '{}: {}'.format(type(exc).__name__, str(exc)),
    })
`;
...
export class ExecutionService {
  static async run(
    ...
    switch (language) {
      ...
      case 'TYPESCRIPT':
        return this.runTypeScript(code, testCases);
      case 'PYTHON':
  ...
  static async runJavaScript(code: string, testCases: TestCase[]): Promise<ExecutionResult> {
    ...
    try {
      const script = new vm.Script(code, { filename: 'student_solution.js' });
      script.runInContext(context, { timeout: 2000 });

      let targetFn: any = null;
      const candidates = extractJavaScriptFunctionCandidates(code);
      ...
      for (const name of candidates) {
        try {
          ...
          // Keep looking for the next user-declared function.
        }
      }
  ...
  static async runPython(code: string, testCases: TestCase[]): Promise<ExecutionResult> {
    ...
    return new Promise<ExecutionResult>((resolve) => {
      const child = spawn(pythonBin, ['-I', '-S', '-c', PYTHON_RUNNER], {
      ...
      child.on('close', () => {
        ...
        try {
          ...
          finish({
            ...
            passedTests: Number(parsed.passedTests || 0),
            totalTests: Number(parsed.totalTests ?? testCases.length),
            testResults: Array.isArray(parsed.testResults) ? parsed.testResults : [],
            stdout: String(parsed.stdout || ''),
            stderr: String(parsed.stderr || stderr || ''),
            executionTimeMs: Date.now() - startTime,
          });
        } catch {
          finish(errorResult(
